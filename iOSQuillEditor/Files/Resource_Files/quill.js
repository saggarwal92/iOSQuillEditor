...
Document = (function() {
  ...
  Document.prototype.optimizeLines = function() {
      //console.log('doc.optimizelines')
    return _.each(this.lines.toArray(), function(line, i) {
        //console.log('pre optimize',line)
      line.optimize();
        //console.log('post optimize',line)
      return true;
    });
  };

  Document.prototype.rebuild = function() {
        //console.log('in doc.rebuild', this);
    ...
    _.each(lines, (function(_this) {
      return function(line, index) {
        ...
        while (line.node !== lineNode) {
          ...
          //console.log('doc.rebuild', line.outerHTML);
          //console.log('doc.rebuild', lineNode.outerHTML);
        if (line.outerHTML !== lineNode.outerHTML) {
          line.node = _this.normalizer.normalizeLine(line.node);
            //console.log('pre line rebuild',line.node.outerHTML);
          line.rebuild();
            //console.log('post line rebuild',line.node.outerHTML);
        }
...
Editor = (function() {
  ...
  function Editor(root, quill, options) {
    ...
    this.selection = new Selection(this.doc, this.quill);
      //console.log('interval',this.options.pollInterval)
  ...
  Editor.prototype._update = function() {
    ...
    if (this.innerHTML === this.root.innerHTML) {
      ...
      //console.log('update',this.root.innerHTML)
    delta = this._trackDelta((function(_this) {
      return function() {
...
//      type: Format.types.LINE,
//      exclude: 'list',
//      preDefParentClass: 'checklist-ul',
////      preDefClass: 'checklist-li',
//      preElement: {
//          'tag':'INPUT',
//          'attributes':{
//              'type':'checkbox',
...
//      attribute:'checked',
  ...
  Format.prototype.formatCheckBox = function(node, value) {
      //console.log(this.config);
    if(value && this.config.preElement){
        //value is true when line break & next element is created also.
        var preNode = document.createElement(this.config.preElement.tag)
        for(attr in this.config.preElement.attributes){
    //            if(attr != 'class')
                preNode.setAttribute(attr,this.config.preElement.attributes[attr])
        }
    //            dom(preNode).addClass('author-galadriel')
        //console.log(node.firstChild.outerHTML);
        if(node.firstChild && node.firstChild.tagName == 'BR'){
            node.html = '';
            var inpBoxes =node.getElementsByTagName('input');
            for(var i=0; i< inpBoxes.length; i++){
                //console.log(i, inpBoxes[i])
                if(inpBoxes[i].type=='checkbox'){
                    inpBoxes[i].remove();
                }
            }
            node.appendChild(preNode);
            node.appendChild(document.createElement('BR'))
        }else if(node.firstChild && node.firstChild.tagName != 'INPUT'){
            ...
            node.insertBefore(preNode,node.firstChild)
            //console.log(node.firstChild);
        }
        //console.log(node);
    }else{
//        var preNode = node.getElementsByClassName(this.config.preElement.attributes.class)
//        if(preNode.length){
//            dom(preNode[0]).remove();
//        }
    }
    if(value){
    var CEFlag = false;
    if(node.closest instanceof Function){
        CEFlag = node.closest('.ql-editor').getAttribute('contenteditable');
        if(CEFlag && CEFlag=='true'){
            this.addCheckHandler(node);
        }
    }else if(node.node && node.node.setAttribute instanceof Function){
        CEFlag = node.node.closest('.ql-editor').getAttribute('contenteditable');
        if(CEFlag && CEFlag=='true'){
            this.addCheckHandler(node.node);
        }
    }
    }
  ...
  Format.prototype.add = function(node, value) {
      //console.log('format.prototype.add - ', node, value);
//      console.log(this.config)
      if(this.config && this.config.prepare == 'strikeThrough'){
//          console.log(p)
      }
    var formatNode, inline, parentNode, ref, ref1;
    if (!value) {
      if(this.config.preElement){// && dom(node).hasClass(Format.FORMATS.checklist.preDefClass)){
//      if(dom(node).hasClass(Format.FORMATS.checklist.preDefClass)){
        var config = Format.FORMATS.checklist
        var preNode = node.getElementsByClassName(config.preElement.attributes.class)
        if(preNode.length)
            dom(preNode[0]).remove();
      }
    ...
    if (_.isString(this.config.parentTag)) {
      ...
      if(this.config.preDefParentClass)
        dom(parentNode).addClass(this.config.preDefParentClass);
    ...
    if (_.isString(this.config.style) || _.isString(this.config.attribute) || _.isString(this.config["class"])) {
      ...
      if (_.isString(this.config.attribute)) {
          //to be fxed - happens for CL
          if(node.setAttribute instanceof Function){
            node.setAttribute(this.config.attribute, value);
          }else if(node.node && node.node.setAttribute instanceof Function){
            node.node.setAttribute(this.config.attribute, value);
          }
      }
    //console.log(node.outerHTML);
    //console.log(this.config["class"] ,',', value);
      if (_.isString(this.config["class"])) {
...
//    console.log(node.outerHTML);
    if(this.config.preDefClass){
      dom(node).addClass(this.config.preDefClass);
    }
    if(this.config.parentTag == 'CL'){
        this.formatCheckBox(node, value);
    }
    if( 0 && this.config.preElement){
...
//            dom(preNode).addClass('author-galadriel')
  ...
  Format.prototype.addCheckHandler = function(node){
    var nsf = new Format(Format.FORMATS.strike);
    var unCheckFn = function(nd){
        if(!nd.childNodes.length){
            nsf.remove(nd)
        }else if(nd.tagName == 'S'){
            nsf.remove(nd)
        }else{
            for(var i = 0; i<nd.childNodes.length; i++){
                unCheckFn(nd.childNodes[i]);
            }
        }
    }
    var toggleClassFn = function(node, e){
        if(e.offsetX < 0){
            //checkbox click
            dom(node).toggleClass('checked');
            if(dom(node).hasClass('checked')){
                node.setAttribute('checked', "true")
            }else{
                node.removeAttribute('checked')
                unCheckFn(node)
//                this_line.formatText(0, this_line.length, "strike", false);
            }
        }
        return false;
    }
    var evtHandler = function(e){
        toggleClassFn(node, e);
        e.stopPropagation();
        e.stopImmediatePropagation();
    }
    node.removeEventListener("click", evtHandler, false);
    node.addEventListener('click', evtHandler, false);
    var text = dom(node).text();
    var checked = node.getAttribute('checked')
    if(checked){
        dom(node).addClass('checked');
    }
    if(!text || !text.length){
        dom(node).removeClass('checked');
        node.removeAttribute('checked')
    }
  };
  ...
  Format.prototype.match = function(node) {
    ...
    if (_.isString(this.config["class"])) {
      ...
      for (i = 0, len = ref1.length; i < len; i++) {
        c = ref1[i];
          //console.log('\n',c,'\n');
        if (c.indexOf(this.config["class"]) === 0 || c.indexOf(this.config["checklist"]) === 0) {
          return true;
        }
      }
      return false;
...
Leaf = (function(superClass) {
  ...
  Leaf.isLeafNode = function(node) {
      //console.log(node.outerHTML, dom(node).isTextNode());
      //console.log(node.firstChild)
...
Line = (function(superClass) {
  ...
  Line.prototype.buildLeaves = function(node, formats) {
      var this_line = this;
      //console.log('here here ', node.outerHTML)
        //console.log('pre dink chak',node.outerHTML);
          if(node.tagName == 'LI' && node.parentNode.tagName == 'CL'){
//              Format.
              var leafFormat = this.doc.formats['checklist'];
                if (leafFormat != null) {
                  leafFormat.formatCheckBox(node, true);
                    toggleClassFn = function(node, e){
                        var CEFlag = this_line.doc.root.getAttribute('contenteditable');
                        if(CEFlag){
                            if(e.offsetX < 0){
                                //checkbox click
                                dom(node).toggleClass('checked');
                    ...
                    */
                    if(node.firstChild && node.firstChild.tagName == 'INPUT'){
                        var checkBox = node.firstChild;
                        dom(checkBox).on('click', function(e){
                            var CEFlag = this_line.doc.root.getAttribute('contenteditable');
                            if(CEFlag){
                                ...
                                this_line.formatText(1, this_line.length, "strike", checked);
                                //console.log(checked);
                                return true;
                            }else{
                                return false;
        ...
        //console.log('post dink chak',node.outerHTML);
    return _.each(dom(node).childNodes(), (function(_this) {
      return function(node) {
        var nodeFormats;
        //console.log('pre doc nornmailzer ',node.outerHTML)
        node = _this.doc.normalizer.normalizeNode(node);
        //console.log('post doc nornmailzer ',node.outerHTML)
...
//              console.log(format.config.exclude)
            if(format.config.exclude && format.config.exclude.length){
            _.each(format.config.exclude, function(exFormat){
              if (exFormat && _this.formats[exFormat]) {
                excludeFormat = _this.doc.formats[exFormat];
                if (excludeFormat != null) {
                  _this.node = excludeFormat.remove(_this.node);
                  delete _this.formats[exFormat];
                }
              }
            });
          }
          if (0 && format.config.exclude && _this.formats[format.config.exclude]) {
          ...
          //console.log('just formatting',_this.node, value)
          _this.node = format.add(_this.node, value);
            if(_this.node.firstChild && _this.node.firstChild.tagName == 'INPUT'){
                var checkBox = _this.node.firstChild;
                dom(checkBox).on('click', function(e){
            //                line = Editor.doc.findLine(this);
                    var checked = this.checked;
                    this_line.formatText(1, this_line.length, "strike", checked);
                    //console.log(checked);
                    return true;
  ...
  Line.prototype.formatText = function(offset, length, name, value) {
      //console.log(offset, length, name, value);
    ...
    while ((leaf != null) && length > 0) {
      ...
      if ((value && leaf.formats[name] !== value) || (!value && (leaf.formats[name] != null))) {
        ...
        if (leaf.length > leafOffset + length) {
          ...
          //console.log('text formatting',targetNode, value)
  ...
  Line.prototype.insertEmbed = function(offset, attributes) {
    ...
    if(attributes['image'].indexOf('http')!=0){
//console.log(attributes['image'])
/*
        Cache.fetchData(
            'upload_image_to_s3',
            'upload_image_ts_'+new Date().getTime().toString(),
            {'data_uri': attributes['image'],'oid':Core.instance.id},
            function(data){
              var oid = this.oid
              $(node).attr('src',data)
              var ins = this.ins
              ins.autosave_note(oid,function(data){})
            },
            Cache.fetchState['ajax_render_cacheupdate'],
            {'page':Core.instance.page,'elem':node,'oid':Core.instance.id,'ins':Core.instance},
            {'type':'POST'}
        )
*/
    }
  ...
  Line.prototype.optimize = function() {
      //console.log('line.optimize',this.node.outerHTML);
    Normalizer.optimizeLine(this.node);
    return this.rebuild();
  };

  Line.prototype.rebuild = function(force) {
      //console.log('in line.rebuild', force);
    if (force == null) {
      force = false;
    }
    ...
    this.node = this.doc.normalizer.normalizeNode(this.node);
      ...
      //console.log(this);
      if(this.node.outerHTML.indexOf('ppy')>=0){
//          //console.log(p)
      }
...
Normalizer = (function() {
  ...
  Normalizer.prototype.whitelistTags = function(node) {
      //console.log(this.whitelist.tags)
      //console.log(node.outerHTML, !dom(node).isElement());
      //console.log(Normalizer.ALIASES[node.tagName],',',this.whitelist.tags[node.tagName]);
      //console.log(dom.BLOCK_TAGS[node.tagName], ',',!node.hasAttributes(),',',node.firstChild );
    if (!dom(node).isElement()) {
      return node;
    }
    if (Normalizer.ALIASES[node.tagName] != null) {
      node = dom(node).switchTag(Normalizer.ALIASES[node.tagName]);
        ...
        //console.log('no line breaks coming here')
    ...
    }else if(node.tagName == 'LI' && node.parentNode.tagName == 'CL'){
  ...
  Normalizer.optimizeLine = function(lineNode) {
      //console.log('normalizer.optimizeLine',lineNode.outerHTML);
    var lineNodeLength, node, nodes, results;
    lineNode.normalize();
      //console.log(lineNode.outerHTML);
    ...
    while (nodes.length > 0) {
      ...
      if ((node != null ? node.parentNode : void 0) == null) {
        ...
        //console.log('continue?', dom.EMBED_TAGS[node.tagName])
  ...
  Normalizer.wrapInline = function(lineNode) {
    ...
    if (lineNode && dom.BLOCK_TAGS[lineNode.tagName] != null) {
      return lineNode;
    }
...
Selection = (function() {
  ...
  Selection.prototype._decodePosition = function(node, offset) {
    var childIndex;
    if (dom(node).isElement()) {
      if(!dom(node).hasClass('checklist-box')){
      ...
      return function(eventName, delta, origin) {
        ...
        if (eventName === _this.quill.constructor.events.TEXT_CHANGE && origin === 'user') {
          ...
          authorFormat = {
            ...
            //console.log('this', _this.options.authorId)
          _.each(delta.ops, function(op) {
            if (op["delete"] != null) {
              return;
            }
...
ImageTooltip = (function(superClass) {
  ...
  ImageTooltip.prototype.handlePasteNoisily = function(evt){
        var e = evt;
        var _this = this;

        if(e.clipboardData.types && (e.clipboardData.types.indexOf('Files') != -1)){
...
//                }, 'img', true);
                ...
                if (range) {
                  _this.textbox.value = '';
                  index = range.end;
                  _this.quill.insertEmbed(index, 'image', dataURI, 'user');

                  _this.quill.setSelection(index + 1, index + 1);
                    /**
                    to do - somehow, selection gets reversed - have to force it forward
                    * */
                    setTimeout(function(){
                      _this.quill.setSelection(index + 1, index + 1);
                    }, 5)
  ...
  ImageTooltip.prototype.initListeners = function() {
    ...
    dom(this.quill.root).on('paste', _.bind(this.handlePasteNoisily, this));
//    dom(this.quill.root).on('drop', function(evt){console.log(evt);});
  ...
  Keyboard.prototype._initEnter = function() {
    ...
    return this.addHotkey(keys, (function(_this) {
      return function(range, hotkey) {
        ...
        //End the list if Enter press at the beginning of the list
        if (offset === 0 && (line.formats.bullet || line.formats.list || line.formats.checklist )) {
          //to do =
          if (line.formats.checklist)
            format = 'checklist'
          else
...
//                console.log(offset)
              if (offset === 0 && (line.formats.bullet || line.formats.list || line.formats.checklist )) {
                  ...
                  if(line.formats.checklist)
                    ...
                    format = line.formats.bullet ? 'bullet' : 'list';
                _this.quill.formatLine(range.start, range.start, format, false);
//              } else if(offset === 1 && (line.formats.checklist)){
//                    format = 'checklist'
//                _this.quill.formatLine(range.start, range.start, format, false);
...
Toolbar = (function() {
  ...
  Toolbar.prototype.initFormat = function(format, callback) {
      //console.log('Toolbar initFormat', format, callback);
  ...
  Toolbar.prototype._applyFormat = function(format, range, value) {
    ...
    return _.defer((function(_this) {
      return function() {
        _this.updateActive(range, ['bullet', 'checklist', 'list']);
...
Quill = (function(superClass) {
  ...
  Quill.DEFAULTS = {
    formats: ['align', 'bold', 'italic', 'strike', 'underline', 'color', 'background', 'font', 'size', 'link', 'image','checkbox', 'bullet', 'checklist', 'list'],
    modules: {
      'keyboard': true,
      'paste-manager': true,
      'undo-manager': true
  ...
  Quill.prototype.formatText = function(start, end, name, value, source) {
      //console.log(start, end, name, value, source);
    var delta, formats, ref;
    ref = this._buildParams(start, end, name, value, source), start = ref[0], end = ref[1], formats = ref[2], source = ref[3];
